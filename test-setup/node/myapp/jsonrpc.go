...
func (s *JSONRPCServer) Broadcast(message string, config Config, node host.Host) (interface{}, *jsonrpc.RPCError) {
	...
	SaveTransaction(msg, config, node)
...
func handleJSONRPC(s *JSONRPCServer, config Config, node host.Host) http.HandlerFunc {
		...
		case "Node.Broadcast":
			...
			if len(req.Params) > 0 {
				...
				if message, ok := req.Params[0].(string); ok {
					...
					result, rpcErr = s.Broadcast(message, config, node)
				} else {
...
func StartJSONRPCServer(port int, server *JSONRPCServer, config Config, node host.Host) {
	http.HandleFunc("/rpc", handleJSONRPC(server, config, node))
